...
if [ -d "android-ndk-${v_ndk}" ]; then
	...
	$WGET "https://dl.google.com/android/repository/android-ndk-${v_ndk}-${os_ndk}.zip"
	unzip -q "android-ndk-${v_ndk}-${os_ndk}.zip"
	rm "android-ndk-${v_ndk}-${os_ndk}.zip"
fi

# Symlink to create expected directory structure for AGP
if [ ! -d "android-sdk-$os/ndk/${v_ndk_n}" ]; then
	echo "Creating NDK symlink inside Android SDK."
	mkdir -p "android-sdk-$os/ndk"
	ln -sf "$PWD/android-ndk-${v_ndk}" "android-sdk-$os/ndk/${v_ndk_n}"
fi
