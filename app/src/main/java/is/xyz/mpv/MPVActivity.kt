...
internal class TouchGestures(private val observer: TouchGesturesObserver) {
    ...
    companion object {
        ...
        // Long press duration used for YouTube-style 2x playback.
    ...
    private fun processMovement(p: PointF): Boolean {
        ...
        // Cancel long press if the finger moves far enough before the timeout.
        if (!longPressActive && state == State.Down) {
            val distance = PointF(
                p.x - initialPos.x,
                p.y - initialPos.y
            ).length()
        ...
        when (state) {
            ...
            State.LongPressSpeed -> {
                // Intentionally do nothing while held.
                // Playback remains at 2x until ACTION_UP.
            }
        }
